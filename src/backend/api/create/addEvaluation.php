...
VALUES
  (?, ?, ?, ?, ?)");
    ...
    if ($stmt->execute()) {
        echo json_encode(array('status' => 'success', 'message' => 'L\'évaluation a été ajouté avec succès.'));
        http_response_code(200);
    } else {
        $error = $connexion->error;
        echo json_encode(array('status' => 'error', 'message' => 'Erreur, l\'évaluation n\'a pas pu être ajouté.', 'error' => $error));
        http_response_code(200);
    }
    $stmt->close();
